...
_respawnText ctrlSetStructuredText parseText ("Welcome to Austerror Survivor<br/>Please click respawn.");
...
buttonSetAction [respawn_Random_Button, "[] execVM 'client\functions\spawnRandom.sqf'"];
/*
_setPlayersInfo =
{
...
// Function to determine the player threshold for use by BIS_fnc_sortBy (friendly = +1, enemy = -1)
_getPlayerThreshold =
{
...
/*	
	if (showBeacons) then
	{
		...
		_locations = [_allowedBeacons, [], {count _allowedBeacons}, "DESCEND", {alive _x}] call BIS_fnc_sortBy;
	}
	else
	{
		_towns = [];
