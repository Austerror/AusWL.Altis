...
if (count _this == 0) then {
    _container = call aus_jerrycan_nearest_pump;
} else {
    _container = _this select 0;
};
_hasemptyjerrycan = "sc_fuelcanisterempty" in magazines player;
_error = "";
switch (true) do {
    case isNull(_container): {_error = ERR_NO_PUMP};
    case (vehicle player != player): {_error = ERR_IN_VEHICLE};
	case (player distance _container > 3): {_error = ERR_NO_PUMP};
	case (!_hasemptyjerrycan): {_error = ERR_NO_JERRYCANS};
    default {};
};
_error;