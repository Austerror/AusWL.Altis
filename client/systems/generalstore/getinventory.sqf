...
_amount = MF_ITEMS_MATCHES call mf_inventory_count;
for [{_x=1},{_x<=_amount},{_x=_x+1}] do
{
	playerInventory set [_index, "matches"];
	_index = _index + 1;
};

_amount = MF_ITEMS_AXE call mf_inventory_count;
for [{_x=1},{_x<=_amount},{_x=_x+1}] do
{
	playerInventory set [_index, "axe"];
	_index = _index + 1;
};

_amount = MF_ITEMS_FIREWOOD call mf_inventory_count;
for [{_x=1},{_x<=_amount},{_x=_x+1}] do
{
	playerInventory set [_index, "firewood"];
	_index = _index + 1;
};
