...
private ["_car", "_additionArray", "_nightTime", "_weapon", "_mag", "_additionOne", "_additionTwo", "_additionThree", "_buildingLootOn", "_random", "_scItems"];
...
_scItems = ["sc_waterbottle","sc_bakedbeans","sc_axe","sc_knife","sc_fuelcanisterempty","sc_fuelcanisterfull"];
...
switch (["A3W_vehicleLoot", 1] call getPublicVar) do
{
    case 1:
    {
		...
		_car addMagazineCargoGlobal [_scItems call BIS_fnc_selectRandom, 1];
		if (_random >= 0.2 || {!_buildingLootOn}) then {_car addMagazineCargoGlobal ["sc_cartyre", 1]};
    ...
    case 2:
    {
        ...
        if (_nightTime) then { _car addMagazineCargoGlobal [_additionThree, 1] };
		_car addMagazineCargoGlobal [_scItems call BIS_fnc_selectRandom, 1];
		if (_random >= 0.2 || {!_buildingLootOn}) then {_car addMagazineCargoGlobal ["sc_cartyre", 1 + floor random 1]};
    };
    case 3:
    {
        _car addWeaponCargoGlobal [_weapon, 1];
