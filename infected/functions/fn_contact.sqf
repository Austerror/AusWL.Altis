...
if ((_zombie distance player) < 5) then {
	...
	if (abs(_targetAngle - _eyeAngle) < 90) then {
		...
		if (!(terrainIntersectASL[eyePos _target, eyePos _zombie])) then {
			if (!(lineIntersects[eyePos _target, eyePos _zombie])) then {
				...
				if (stance _target == "STAND") then {
					if ((_target distance _zombie) < (20+(60*_fogc))) then {
						//hintSilent formatText["Spotted: %1, Distance: %2", name _target, _target distance _zombie];
						_return = true;
					};
				};
				if (stance _target == "CROUCH") then {
					if ((_target distance _zombie) < (45+(35*_fogc))) then {
						//hintSilent formatText["Spotted: %1, Distance: %2", name _target, _target distance _zombie];
						_return = true;
					};
				} else {
					if ((_target distance _zombie) < (40+(10*_fogc))) then {
						//hintSilent formatText["Spotted: %1, Distance: %2", name _target, _target distance _zombie];
						_return = true;
					};
				};
