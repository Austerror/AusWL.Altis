...
if(isserver)then{
	...
	_finalTarget=[];
	_playerList=[];
	_targetList = _me nearEntities [["Car", "Man"], 150];
	{
		  //if (isPlayer _x && (alive _x) ) then { _playerList = _playerList + [_x] };		//List BLUFOR players only
		if ((alive _x )and( side _x != resistance )) then //List all but GUER units (don't want infected fighting each other... or do we?)
		{
