private ["_me","_this","_myTarget","_myzone","_initialPos","_imHere","_target","_isInVehicle","_targetPos","_distance","_asleep","_killCount","_damageDone"];
if (isServer)then
{
	_me=_this select 0;
	...
	while{(alive _me)and(count _myTarget != 0)}do
	{
		...
		{	
			if ((alive _me) and (alive _target))then
			{
				...
				_target setDamage (damage _target + 0.08);
