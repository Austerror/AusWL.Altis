private ["_me","_this","_myzone","_initialPos","_imHere","_targetList","_targetCount","_target","_isInVehicle","_targetPos","_distance","_asleep","_damageDone"];
if(isServer)then
{
	_me=_this select 0;
	...
	while{alive _me}do
	{
		if(!alive _target)then
		{
			...
			if((_distance<=20)and(!_imHere))then
			{
				...
				[[_me,"idle",_target], "INF_fnc_NextSound"] call BIS_fnc_MP;
				sleep 1;
			};
			...
			{	
				if ((alive _me) and (alive _target))then
				{
					...
					sleep 0.2;
					_damageDone = floor random 0.2;
					_target setDamage (damage _target + _damageDone);
					//[_me, "punch"] spawn INF_fnc_NextSound;
					[[_me,"punch",_target], "INF_fnc_NextSound"] call BIS_fnc_MP;
					sleep 2;
				};
			};
		};
