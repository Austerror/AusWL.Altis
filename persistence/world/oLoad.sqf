		...
		for "_i" from 1 to _objectsCount do
		{
			...
			if (!isNil "_class" && {!isNil "_pos"} && {_maxLifetime <= 0 || {_hoursAlive < _maxLifetime}}) then 
			{
				...
				if (_allowed) then
				{
					...
					_vehFuel = _obj getVariable ["vehicleFuel", -1];
					if (_vehFuel != -1) then {
						_obj setFuel _vehFuel;
					};
					_vehTex = _obj getVariable ["vehicleTexture", ""];
					if (_vehTex != "") then {
						[_obj, _vehTex] call aus_applyTexture;
					};
					if (_obj isKindOf "AllVehicles") then {
						_obj allowDamage true;
						_indDamage = [_fileName, _objName, "IndDamage", "ARRAY"] call iniDB_read;
						{
							_obj setHitPointDamage [_x select 0, _x select 1];
						} forEach _indDamage;
						if ((_obj isKindOf "Helicopter") && (typeOf _obj != "CSJ_GyroC")) then {
							objectCountHelis = objectCountHelis + 1;
						};					
						if ((_obj isKindOf "Car") || (_obj isKindOf "Tank") || (typeOf _obj == "CSJ_GyroC")) then {
							objectCountVehicles = objectCountVehicles + 1;
						};
						if (_obj isKindOf "Plane") then {
							objectCountPlanes = objectCountPlanes + 1;
						};
					if (_obj isKindOf "Ship") then {
							objectCountBoats = objectCountBoats + 1;
						};							
					};
