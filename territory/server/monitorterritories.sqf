...
_onCaptureFinished =
{
    ...
    _otherSideObjects = [west, east, resistance, civilian] - [_sideObject];
...
_sideObjectForSideStr =
{
    ...
    switch (_sideStr) do
	{
        ...
        case "CIV": { _sideObj = civilian };
        case "ENEMY": { _sideObj = civilian };
    };
...
_nameForSideStr =
{
    ...
    switch (_side) do
	{
        ...
        case "CIV":  { _sideName = "Civilian" };
        case "ENEMY":  { _sideName = "Civilian" };
        default      { _sideName = "" };
    };
...
_markerColorForSideStr =
{
    ...
    switch (_side) do
	{
        ...
        case "CIV": { _markerColor = "colorpurple" };
        case "ENEMY": { _markerColor = "colorpurple" };
...
{ 
    ...
    private ['_players', '_blueCount', '_redCount', '_greenCount', '_purpleCount', '_contested'];
	...
	{
        {
            switch (side _x) do
			{
                ...
                case civilian: 	 { _purpleCount = _purpleCount + 1 };
            };
        } forEach _players;

        if ((_blueCount > 0 && _redCount > 0) ||
            (_redCount > 0 && _purpleCount > 0) ||
            (_purpleCount > 0 && _greenCount > 0) ||
		...
		switch (true) do
		{
			...
			case (_purpleCount > 0 && !_contested): { _dominantSide = "CIV" };
		}
    };

    [_blueCount, _redCount, _greenCount, _purpleCount, _contested, _dominantSide]
};
...
_handleSideCounts =
{
    ...
    _currentPurpleCount = _currentSideCounts select 3;
    _currentAreaContested = _currentSideCounts select 4;
    _currentDominantSide = _currentSideCounts select 5; 

    _newBlueCount = _newSideCounts select 0;
    _newRedCount = _newSideCounts select 1;
    _newGreenCount = _newSideCounts select 2;
    _newPurpleCount = _newSideCounts select 3;
    _newAreaContested = _newSideCounts select 4;
    _newDominantSide = _newSideCounts select 5;
...
_handleCapPointTick = {
    ...
    for "_i" from 0 to (_count - 1) do
	{
        ...
        if (count _newTerritoryData > 0 && { count _newTerritoryDetails > 0 }) then
		{
            ...
            _currentDominantSide = _currentSideCounts select 5;
            _newDominantSide = _newSideCounts select 5;
            _newContestedStatus = _newSideCounts select 4;
