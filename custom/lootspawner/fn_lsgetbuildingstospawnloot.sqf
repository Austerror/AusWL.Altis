...
//-------------------------------------------------------------------------------------
private["_lootInfo", "_lootChance", "_lootRoll", "_begintime","_BaP_list","_spInterval","_chfullfuel","_chpSpot","_genZadjust","_BaPname","_lootClass","_buildPosViable_list","_buildPosZadj_list","_lBuildVar","_timeStamp","_posviablecount","_spwnPos","_lootspawned","_randChance","_lootholder","_selecteditem","_loot","_chfullf","_idx_sBlist","_chperSpot","_tmpPos"];
				...
				if ((floor random 100) < _chpSpot) then {
					...
					if ((count (nearestObjects [_spwnPos, LSusedclass_list, 0.5])) == 0) then {
						...
						switch (_lootType) do
						{
							...
							case 1:
							{
								_lootRoll = 1;  // _lootChance (1 = every time, 100 = rarest)
								_lootChance = 999;
								while {_lootChance > _lootRoll} do {
									_lootInfo = ((lootWeapon_list select _lootClass) select 1) call BIS_fnc_selectRandom;
									_loot = _lootInfo select 0;
									_lootChance = _lootInfo select 1;
									_lootRoll = 1 + floor random 100;									
								};
							...
							case 2:
							{
								...
								for "_rm" from 1 to _randChance do {
									...
									_lootChance = 999;
									while {_lootChance > _lootRoll} do {
										_lootInfo = ((lootMagazine_list select _lootClass) select 1) call BIS_fnc_selectRandom;
										_loot = _lootInfo select 0;
										_lootChance = _lootInfo select 1;
										_lootRoll = 1 + floor random 100;
									};
									_lootholder addMagazineCargoGlobal [_loot, 1];
//									if(_loot == "sc_matchbook") then {
//										_loot setVariable["matchcount", 5 + (random 25), true];
//									};									
								};
							};
							...
							case 3:
							{
								...
								_lootChance = 999;
								while {_lootChance > _lootRoll} do {
									_lootInfo = ((lootItem_list select _lootClass) select 1) call BIS_fnc_selectRandom;
									_loot = _lootInfo select 0;
									_lootChance = _lootInfo select 1;
									_lootRoll = 1 + floor random 100;
								};							
							...
							case 4:
							{
								...
								_lootChance = 999;
								while {_lootChance > _lootRoll} do {
									_lootInfo = ((lootBackpack_list select _lootClass) select 1) call BIS_fnc_selectRandom;
									_loot = _lootInfo select 0;
									_lootChance = _lootInfo select 1;
									_lootRoll = 1 + floor random 100;
								};							
							...
							case 5:
							{
								...
								if(_loot == "Land_Canteen_F") then {
									_chfullf = (random 100);
									if (_chfullfuel > _chfullf) then {
										_lootholder setVariable["mf_item_id", "canteenfull", true];
									} else {
										_lootholder setVariable["mf_item_id", "canteenempty", true];
									};
								};
								if(_loot == "Land_CanisterOil_F") then {
									_lootholder setVariable["mf_item_id", "syphonhose", true];
								};
								if(_loot == "Land_Matches_F") then {
									_lootholder setVariable["mf_item_id", "matches", true];
								};
								if(_loot == "Land_Axe_F") then {
									_lootholder setVariable["mf_item_id", "axe", true];
								};
								if(_loot == "Land_CanOpener_F") then {
									_lootholder setVariable["mf_item_id", "knife", true];
								};
								if(_loot == "Land_Woodenlog_F") then {
									_lootholder setVariable["mf_item_id", "firewood", true];
								};
