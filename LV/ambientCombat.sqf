...
_sideRatios = if(count _this > 5)then{_this select 5;} else {[0,1,1]}; 
_syncedUnit = if(count _this > 6)then{_this select 6;} else {player};	 
_skills = if(count _this > 7)then{_this select 7;} else {0.2};	 
...
_patrolType = if(count _this > 11)then{_this select 11;} else {["AWARE","SAD"]};	
...
if(isNil("LV_infectedGroup"))then{LV_infectedGroup = compile preprocessFile "LV\LV_functions\LV_fnc_infectedGroup.sqf";};
...
if(isNil("LV_ReGear"))then{LV_ReGear = compile preprocessFile "LV\LV_functions\LV_fnc_regear.sqf";};
if(isNil("LV_Killed"))then{LV_Killed = compile preprocessFile "LV\LV_functions\LV_fnc_killed.sqf";};
...
if(isNil("LV_AI_infectedGroups"))then{LV_AI_infectedGroups = [];};
...
while{true}do{
	...
	if(count LV_ACS_activeGroups < _groupAmount)then{
		...
		_lossRatio = _groupAmount - _westRatio - _eastRatio;
				//hint format["%1",(side (LV_ACS_activeGroups select (count LV_ACS_activeGroups - 1)))];
		_westGroups = {(side _x) == resistance} count LV_ACS_activeGroups;
		...
		if(_menOrVehicle < 4)then{
			if(surfaceIsWater _spawnPos)then{
				...
				_landOrAir = floor(random 10);
				if(_landOrAir < 4.5)then{
			...
			if(surfaceIsWater _spawnPos)then{
				//_grp = [_spawnPos, _side, [1,2]] call LV_diveGroup;
				_grp = [_spawnPos, _side] call LV_fullAirVehicle;
			}else{
				_infectedChance = random floor 100;
				if (_infectedChance > 40) then {
					_grp = [_spawnPos, 3, [3,10]] call LV_infectedGroup;
					//{
					//	[_x,"null = this spawn INF_fnc_infecthim;"] spawn LV_vehicleInit;
					//} forEach units group _grp;
				} else {
					_grp = [_spawnPos, _side, [1,4]] call LV_menGroup;
				};
			};
		};
		...
		switch(_side)do{
			...
			case 3:{
				LV_AI_infectedGroups set [(count LV_AI_infectedGroups), (group _grp)];
			};
		};
